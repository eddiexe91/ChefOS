...
begin;
do $$
declare u uuid:=gen_random_uuid(); r uuid; p uuid; salida uuid; rec public.recetas; stock numeric; clave text; miembro uuid:=gen_random_uuid();
begin
  ...
  insert into public.productos(restaurante_id,nombre,unidad_medida,stock_actual,cantidad_gramos,stock_minimo,stock_minimo_gramos,tipo_operativo)
  values(r,'QA Caldo listo','kg',0,0,1,1000,'elaborado') returning id into salida;
  rec:=public.crear_receta_completa(r,u,jsonb_build_object('nombre','QA Caldo','rendimiento_porciones',2,'unidad_rendimiento','porcion','en_carta',true,'es_produccion',true,'producto_salida_id',salida,'cantidad_salida',1,'unidad_salida','kg','ingredientes',jsonb_build_array(jsonb_build_object('producto_id',p,'cantidad',1,'unidad_medida','kg','orden',0)),'pasos',jsonb_build_array(jsonb_build_object('numero',1,'titulo','Cocer','descripcion','Cocer ingredientes'))));
  if rec.id is null or rec.producto_salida_id is distinct from salida or rec.cantidad_salida_gramos is distinct from 1000::numeric then raise exception 'QA salida no guardada'; end if;
  ...
  select cantidad_gramos into stock from public.productos where id=salida;
  if stock<>1000 then raise exception 'QA salida incorrecta: %',stock; end if;
  if not exists(select 1 from public.actividad_operativa where restaurante_id=r and accion='registrar_produccion') then raise exception 'QA actividad ausente'; end if;
  perform public.registrar_merma_completa(r,salida,0.25,'kg','otro',u,'QA');
  select cantidad_gramos into stock from public.productos where id=salida;
  if stock<>750 then raise exception 'QA merma incorrecta: %',stock; end if;
end $$;
rollback;
select 'PASS: equipo, receta/carta, consumo 1000g, salida 1000g, actividad y merma 250g. Datos QA revertidos.' as resultado;
