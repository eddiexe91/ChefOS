...
export default function NuevoProductoCliente({ onClose }: Props) {
  ...
  return <div className="fixed inset-0 z-50 bg-black/60 flex items-end justify-center" role="dialog" aria-modal="true" aria-label="Nuevo producto">
    <form onSubmit={guardar} className="w-full max-w-lg mb-[calc(64px+env(safe-area-inset-bottom))] rounded-2xl bg-fondo-elevado p-5 space-y-4 pb-5">
      ...
      <select value={categoriaId} onChange={(e) => setCategoriaId(e.target.value)} className="w-full min-h-11 rounded-xl border border-fondo-borde bg-fondo-card px-3 text-sm text-texto-primario"><option value="">Categoría del producto</option>{(categorias.data ?? []).map((categoria: CategoriaProducto) => <option key={categoria.id} value={categoria.id}>{categoria.nombre}</option>)}</select>
