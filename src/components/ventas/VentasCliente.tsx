...
export default function VentasCliente() {
  ...
  return <div className="px-4 pt-6 pb-28 space-y-5 max-w-lg mx-auto">
    ...
    <div className="space-y-3">{importaciones.map((item) => {
      const historico = item.modo === 'historico'
      const href = historico ? (item.estado_procesamiento === 'completado' ? '/ventas/mapeos' : '/ventas/importar') : `/ventas/${item.id}`
      return <Link href={href} key={item.id} className="block"><article className="tarjeta p-4"><div className="flex justify-between gap-3"><div><p className="text-sm text-texto-primario">{item.fecha_inicio} — {item.fecha_fin}</p><p className="text-xs text-texto-apagado mt-1">{item.total_registros} registros · {historico ? 'Historial: sin descuento de inventario' : `${item.registros_pendientes} por revisar`}</p></div><span className="badge-acento">{item.estado_procesamiento}</span></div><p className="text-xs text-acento mt-3">{historico ? (item.estado_procesamiento === 'completado' ? 'Revisar equivalencias POS' : 'Ir al importador histórico') : 'Abrir revisión'} →</p></article></Link>
    })}</div>
  </div>
}
