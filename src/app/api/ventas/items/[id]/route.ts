...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const { data: item } = await supabase.from('ventas_items').select('*').eq('id', params.id).eq('restaurante_id', perfil.restaurante_id).single()
  if (!item) return NextResponse.json({ error: 'Venta no encontrada.' }, { status: 404 })
  if (item.ticket_id) return NextResponse.json({ error: 'Los productos históricos se vinculan desde Productos POS, no modificando cada venta.' }, { status: 409 })
