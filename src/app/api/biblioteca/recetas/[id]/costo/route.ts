...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
  ...
  const { data: perfil } = await supabase.from('usuarios').select('restaurante_id, activo, rol').eq('id', user.id).eq('activo', true).single()
  if (!perfil) return NextResponse.json({ data: null, error: 'Usuario no encontrado.' }, { status: 401 })
  if (!['dueño', 'chef_ejecutivo', 'chef_cocina'].includes(perfil.rol)) return NextResponse.json({ data: null, error: 'Sin permisos para recalcular costos.' }, { status: 403 })
