...
export async function POST(request: NextRequest) {
  ...
  const { data: merma, error: mermaError } = await supabase
    ...
    .insert({
      ...
      motivo:          datos.motivo,
      responsable_id:  datos.responsable_id ?? perfil.id,
      notas:           datos.notas,
    })
    .select()
    .single()
  ...
  if (mermaError || !merma) {
    console.error('[ChefOS/api/mermas] Error al insertar merma:', mermaError?.message)
    return errorJSON('Error al registrar la merma. Intenta nuevamente.', 500)
  }

  // ── 7. Insertar movimiento de inventario ────────────────────
  ...
  if (movimientoError) {
    console.error('[ChefOS/api/mermas] Error al insertar movimiento:', movimientoError.message)
    return errorJSON('Error al registrar el movimiento de inventario. Intenta nuevamente.', 500)
  }
