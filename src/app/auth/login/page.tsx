  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      if (modo === 'magic-link') {
        ...
        // Capacitor/WebView puede tardar un instante en persistir las cookies
        // SSR. Validamos la sesión y el perfil antes de navegar para evitar
        // que el usuario quede aparentemente detenido en el formulario.
        const { data: sesion } = await supabase.auth.getSession()
        if (!sesion.session?.user) {
          throw new Error('La sesión no pudo guardarse. Comprueba tu conexión y vuelve a intentarlo.')
        }
        ...
        if (!perfil?.restaurante_id || perfil.activo === false) {
          throw new Error('La cuenta se autenticó, pero todavía no tiene un restaurante activo. Vuelve a crear el restaurante o contacta al administrador.')
        }

        const sincronizarSesion = await fetch('/api/auth/session', {
          method: 'POST',
        ...
        if (!sincronizarSesion.ok) {
          const detalle = await sincronizarSesion.json().catch(() => ({})) as { error?: string }
          throw new Error(detalle.error ?? 'No se pudo sincronizar la sesión con ChefOS.')
        }

        // La navegación completa fuerza a Next.js a leer las cookies recién
        // creadas; es más fiable que router.push dentro de Android WebView.
    ...
    <div className="min-h-dvh bg-fondo-base flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-sm">
        ...
        <p className="text-center text-xs font-sans text-texto-apagado mt-8">
          ¿Primera vez? <Link href="/auth/registro" className="text-acento underline">Crear restaurante</Link>
        </p>
      </div>
    </div>
  )
