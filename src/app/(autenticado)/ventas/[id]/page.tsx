...
export default function RevisionVentasPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => { fetch(`/api/ventas/importaciones/${params.id}`).then((r) => r.json()).then((j: { data?: { importacion?: { modo?: string }; items: Item[]; recetas: Receta[] } }) => { setHistorico(j.data?.importacion?.modo === 'historico'); setItems(j.data?.items ?? []); setRecetas(j.data?.recetas ?? []) }) }, [params.id])
